...
void mouseSingleClick(int button, int action) { //鼠标单次点击
	if (button == GLFW_MOUSE_BUTTON_LEFT && action == GLFW_PRESS) { //左键按下
		...
		//一直闪动
		reinterpret_cast<UI::BorderMaskUI*>(UI::UIManager::getUI(UI::UIID::BORDER_MASK))->reset();
	...
	if (button == GLFW_MOUSE_BUTTON_MIDDLE && action == GLFW_PRESS) { //中键按下
		//闪动一次
		reinterpret_cast<UI::BorderMaskUI*>(UI::UIManager::getUI(UI::UIID::BORDER_MASK))->reset(1);
		UI::UIManager::setUIVisable(UI::UIID::BORDER_MASK, true);
