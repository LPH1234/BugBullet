...
void Media::PlayMedia3D(vec3df pos, Media::MediaType _type) {
	...
	if (_type == Media::MediaType::EXPLODE) {
		...
		Sound = SoundEngine->play3D("audio/aimed.wav", pos / 5, false, false, true, ESM_AUTO_DETECT, true);
		Sound->setVolume(0.5);
	}
	else if (_type == Media::MediaType::TANKSHOOT) {
		...
		Sound = SoundEngine->play3D("audio/rlaunch.wav", pos / 30, false, false, true, ESM_AUTO_DETECT, true);
	...
	else if (_type == Media::MediaType::SWITCH_AMMO) {
		Sound = SoundEngine->play3D("audio/switchAmmo.wav", pos / 5, false, false, true, ESM_AUTO_DETECT, true);
		//Sound->setVolume(0.7f);
	}
}
...
void Media::setEnableEffect(bool enable) {
